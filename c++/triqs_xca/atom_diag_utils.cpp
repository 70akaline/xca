  ...
  namespace {
      ...
      for (int oidx = 0; oidx < n; ++oidx) {
        ...
        for (int sidx = 0; sidx < ad.n_subspaces(); ++sidx) {
          ...
          if (cidx >= 0) {
            ...
            auto c_full = get_full_operator_matrix_impl(ad, oidx, false);
          ...
          if (didx >= 0) {
            ...
            // Get full operator matrix and extract block
            auto cdag_full = get_full_operator_matrix_impl(ad, oidx, true);
            for (int i = 0; i < fock_final.size(); ++i) {
    ...
    get_operators_dense_impl(const triqs_atom_diag_t<IsComplex> &ad) {
      ...
      for (int oidx = 0; oidx < norb; ++oidx) {
        Fs(oidx, cppdlr::_, cppdlr::_)    = get_full_operator_matrix_impl(ad, oidx, false);
        Fdags(oidx, cppdlr::_, cppdlr::_) = get_full_operator_matrix_impl(ad, oidx, true);
      }
      return {Fs, Fdags};
    }
