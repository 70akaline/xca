...
TEST(DenseBackbone, third_order_manual) {
  ...
  for (int f_ix_off = 0; f_ix_off < pow_n_mm1; f_ix_off++) {
    ...
    D.eval_self_energy_fixed_indices(Gt_dense, B, f_ix_start + f_ix_off);
    //B.reset_all_inds();
  }
...
TEST(DenseBackbone, one_fermion_three_orders_hyb_one_pole) {
  double beta   = 1.0;
  double Lambda = 20.0 * beta;
  double eps    = 1.0e-10;
  ...
  ASSERT_LE(nda::max_element(nda::abs(nca_se[0].data()(_, 1, 1) - nca_se_ana(_, 1))), eps);
